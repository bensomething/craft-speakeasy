...
- A second Password field is only called out as having no effect when the first one actually holds a password, since the gate falls through an empty field to a later one.
